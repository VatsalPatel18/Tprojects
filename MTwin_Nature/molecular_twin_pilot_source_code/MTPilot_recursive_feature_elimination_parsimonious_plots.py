...
scoring_metrics = ['balanced_accuracy']
...
learning_curves_df.to_csv("./molecular_twin_pilot/outputs/multiomic/combinations_of_learning_curves_dataframe_tcga_analytes.csv", index=False)
# -

results_df = pd.read_csv("./molecular_twin_pilot/outputs/multiomic/recursive_feature_elimination_learning_curve_runs.csv")
...
def plot_multiomic_learning_curve(key, group_df):
    ...
    pyplot.title(f'Parsimonious Model Learning Curve for {parsimonious_graph_title(analyte, ", ")}', fontsize=28)
    ...
    pyplot.yticks(fontsize=22)
    pyplot.figure(figsize=(64, 32), dpi=400)
    pyplot.savefig(f'./molecular_twin_pilot/MTPilot_learning_curve_for_survival_{parsimonious_graph_title(analyte, "_")}.png')
    pyplot.show()
...
def plot_learning_curve(key, group_df):
    ...
    # Scatter plot of analyte performance at each feature count interval.
    pyplot.rcParams["figure.figsize"]=(64, 32)
